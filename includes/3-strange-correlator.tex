...
\begin{align}
  ...
  &= \delta_{rp'} \sum_{q''s''} F^{sqs'}_{q'rq''} F^{q''s's}_{pqs''} F^{s'sq''}_{r'q's''}
     \sqrt{\frac{d_s d_{s'}}{d_{s''}}} \calT{s''}{pq''r'}, \\
     \bigl( \calT{s}{pqr} \bigr)^\dagger
  &= \, \vcenter{\hbox{\includegraphics{images/mpo/TCTubedagger.pdf}}} \,
   = \sqrt{\frac{d_p}{d_r}} \sum_{q'} F^{srs}_{pqq'} \calT{s}{rq'p}.
...
管状代数可以被分块对角化，其中的每一块上的投影算符对应一个\emph{不可约中心幂等元} (irreducible central idempotent)。从范畴论的角度来看，这些中心幂等元构成了一个新的范畴，它对应了输入范畴 $\mathcal{C}$ 的 \emph{Drinfeld 中心} (Drinfeld center) $\mathcal{Z}(\mathcal{C})$。而从拓扑序的角度来看，它们则对应了不同的\emph{拓扑分区} (topological sectors)\cite{lan2014topological,bultinck2017anyons,williamson2017symmetry,vanhove2018mapping}。

中心幂等元可以表示为管状代数中基的线性组合：
\begin{equation}
  \calP{i} = \frac{1}{D^2} \sum_{pqrs} t^i_{pqrs} \calT{s}{pqr},
\end{equation}
其中 $D$ 为总量子维数，而系数 $t^i_{pqrs}$ 可根据 $\calP{i}$ 满足的约束条件来求解：
\begin{equation}
  \calP{i} \calP{j} = \delta_{ij} \calP{i}, \quad
  \calP{i}^\dagger = \calP{i}, \quad
  \calT{s}{pqr} \calP{i} = \calP{i} \calT{s}{pqr}.
\end{equation}
以包含 $\1$、$\tau$ 两种任意子的 Fibonacci 模型为例\cite{bultinck2017anyons}，它的中心幂等元共有 4 个：
\begin{equation}
  \begin{aligned}
    \calP{1} &= \frac{1}{\sqrt5} \left(
      \frac{1}{\varphi} \calT{\1}{\1\1\1} + \sqrt{\varphi} \calT{\tau}{\1\tau\1}
    \right), \\
    \calP{2} &= \frac{1}{\sqrt5} \left(
      \frac{1}{\varphi} \calT{\1}{\tau\tau\tau} + \frac{1}{\sqrt{\varphi}} \ee^{-4\pi\ii/5} \calT{\tau}{\tau\1\tau} + \ee^{ 3\pi\ii/5} \calT{\tau}{\tau\tau\tau}
    \right), \\
    \calP{3} &= \frac{1}{\sqrt5} \left(
      \frac{1}{\varphi} \calT{\1}{\tau\tau\tau} + \frac{1}{\sqrt{\varphi}} \ee^{ 4\pi\ii/5} \calT{\tau}{\tau\1\tau} + \ee^{-3\pi\ii/5} \calT{\tau}{\tau\tau\tau}
    \right), \\
    \calP{4} &= \frac{1}{\sqrt5} \left(
      \varphi \calT{\1}{\1\1\1} + \calT{\1}{\tau\tau\tau} - \sqrt{\varphi} \calT{\tau}{\1\tau\1} + \sqrt{\varphi} \calT{\tau}{\tau\1\tau} + \frac{1}{\varphi} \calT{\tau}{\tau\tau\tau}
    \right),
  \end{aligned}
...
\begin{equation}
  \calP{1} \mapsto (\1, \1), \quad
  \calP{2} \mapsto (\1, \bar{\tau}), \quad
  \calP{3} \mapsto (\tau, \1), \quad
  \calP{4} \mapsto (\tau, \bar{\tau}),
\end{equation}
相应的拓扑自旋分别为
\begin{equation}
...
\begin{equation}
  g_1 \times g_2 = g_3.
\end{equation}
如图~\ref{fig:dw-triangulation}\,(a) 所示，群乘法也决定了三角形的定向。我们选取圆盘作为底流形，则路径积分将会在圆 $S^1$ 上给出相应二维模型的基态波函数。最简单的三角剖分如图~\ref{fig:dw-triangulation}\,(b) 所示，它可以表示为一个矩阵乘积态：
\begin{equation}
    \ket{\Psi}_G
  = \sum_{\{g_i\}} \sum_{\{h_i\}}
...
\begin{figure}[htb]
  ...
  \begin{tabular}{c@{\qquad}c}
    $\vcenter{\hbox{\includegraphics[width=0.2\textwidth]{images/holographic/dw-triangle.png}}}$ &
    $\vcenter{\hbox{\includegraphics[width=0.5\textwidth]{images/holographic/dw-triangulation.png}}}$ \\
    (a) & (b)
  \end{tabular}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.7\textwidth]{images/holographic/rg-2+1d-blocking.png}
  \caption[边界态 $\bra{\Omega}$ 的 PEPS 张量网络表示]{上图为边界态 $\bra{\Omega}$ 的 PEPS 张量网络表示，其指标位于三角形的边上。每条边都有两个指标，一个是物理指标 $a_i\in\mathcal{C}$，另一个是连接维数为 $\chi$ 的辅助指标 $I_i$。在下图中，我们利用 SVD 把张量 $M(k)$ 分解为两个张量 $\tilde{T}$ 的缩并。指标 $H$（标记为红色）在生成时，连接维数为 $\chi^2$，但我们会将其截断为 $\chi$ 以使得重整化过程得以持续。}
