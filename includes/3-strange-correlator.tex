...
为了与经典二维 Ising 模型相对应，我们考虑一个定义在正方形和八边形交错的格点上、由 $\1$、$\sigma$ 和 $\psi$ 三种简单对象构成的 Ising 弦网模型。各任意子对应的量子维数分别为 $d_{\1}=d_\psi=1$、$d_\sigma=\sqrt2$，融合规则为
\begin{equation}
  \begin{gathered}
...
\end{equation}
上面。为了得到正方形格点的结构，我们用 8 个三角形组成一个带有 4 个指标的八边形张量单元：
\def\DiagramI{
