...
\begin{figure}[htb]
  ...
  \subcaptionbox{Ising 模型}{%
    \includegraphics[width=0.45\textwidth]{images/temp/ising-central-charge.png}} \quad
...
接下来我们以 $\mathcal{A}_{k+1}$ 模型（见 \ref{subsec:a-k+1-category} 小节）为例给出一些具体的数值计算结果。如图~\ref{fig:su2-k-boundary-condition} 所示，$\mathcal{A}_{k+1}$ 中的边界态可以取为
\begin{equation}
...
类似地，三维配分函数也可通过 3+1 维 TQFT 基态波函数与对应的奇异关联子获得。对于 3+1 维 TQFT 而言，输入数据是 \emph{2-融合范畴} (2-fusion category)，而相应张量网络的构建块是\emph{四维单形} (4-simplex)。此时，边会被赋予 2-范畴中的对象，面会被赋予 1-态射，而三维单形则会被赋予 2-态射。
...
具体来说，三维边界由一系列小正方体填充，每个正方体可继续分为 6 个四面体，而顶点则被 8 个正方体共享［图~\ref{fig:3+1d-cube-1}］。注意这样的正方体单元并不是平移不变的，最小可重复的填充单元需要 $2\times2\times2$ 个小正方体［图~\ref{fig:3+1d-cube-2}］。
...
我们考虑 3+1 维 toric code 模型（即 3+1 维 $\mathbb{Z}_2$ Dijkgraaf-Witten 模型），其边界态可以取为
\begin{equation}
