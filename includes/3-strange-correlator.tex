...
弦网模型中的 MPO 张量单元为\footnote{不同文献中张量的具体选取会略有不同，这里与 \ref{sec:string-net-peps} 节相比差了一些全局的系数，但并不影响其结论。式~\eqref{eq:string-net-mpo}--\eqref{eq:tube-algebra-hermitian-conjugate} 中图片来源：\parencite{williamson2017symmetry}。}：
\begin{equation}
