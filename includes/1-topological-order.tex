...
\begin{itemize}
  ...
  \item \emph{弦的取向}，指定了对象 $i$ 的对偶 $i^*$，在弦图中相当于把箭头反向［图~\ref{fig:string-net-orientations-branching-rules} (a)］；
...
\begin{equation}
  ...
  \end{gathered}
\end{equation}
$F^{ijm}_{kln}$ 是 $F$ 符号的另一种记法，它与 $[F^{ijk}_l]_{mn}$ 的关系将在 \ref{...} 中给出。
...
\begin{equation}
    A_v \ket{\tikzinput{string-net-fusion-2}}
  = \delta_{ijk} \ket{\tikzinput{string-net-fusion-2}}, \quad
...
\begin{equation}
  B_p = \sum_{s=0}^N \frac{d_s}{D} B_p^s, \quad D = \sum_{s=0}^N d_s^2.
\end{equation}
这里 $D$ 是总量子维数，而 $B_p^s$ 满足
\begin{align}
     B_p^s \, \Biggl| \tikzinput{string-net-loop-1} \Biggr\rangle
  &= \sum_{m,\dots,r} B_{p,ghijkl}^{s,g'h'i'j'k'l'}(abcdef) \,
     \Biggl| \tikzinput{string-net-loop-2} \Biggr\rangle \notag \\
  &= F^{al^*g}_{s^*g'l'^*}
     F^{bg^*h}_{s^*h'g'^*}
     F^{ch^*i}_{s^*i'h'^*}
     F^{di^*j}_{s^*j'i'^*}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.4\textwidth]{images/temp/topological-entanglement-entropy.pdf}
