...
\begin{figure}[htb]
  ...
  \begin{tabular}{c@{\qquad}c}
    \tikzinput{tensor-network/mps-1} &
    \tikzinput{tensor-network/mps-2} \\
    (a) & (b)
  \end{tabular}
  \caption[矩阵乘积态]{矩阵乘积态。图 (a)、(b) 分别对应开放和周期性边界条件。}
  \label{fig:mps}
\end{figure}
...
\begin{figure}[htb]
  ...
  \caption[规范自由度]{规范自由度。其中张量单元 $\tilde{A}=XAX^{-1}$。}
...
\begin{align}
  ...
  &= \vcenter{\hbox{\includegraphics[height=1.7cm]{images/tensor-network/dmrg-energy-functional.pdf}}}.
...
\begin{equation}
  \vcenter{\hbox{\includegraphics[height=1.9cm]{images/tensor-network/dmrg-energy-equation.pdf}}}.
\end{equation}
由于 MPS 已被正则化（$R$、$L$ 的特征向量均为单位向量），上式中的第二项可以被大大简化：
\begin{equation}
  \vcenter{\hbox{\includegraphics[height=1.9cm]{images/tensor-network/dmrg-normalized-energy-equation.pdf}}}.
...
\begin{figure}[htb]
  \centering
  \begin{tabular}{cc}
    (a) & $\vcenter{\hbox{\includegraphics[width=0.7\textwidth]{images/temp/dmrg-two-site.png}}}$ \\
    (b) & $\vcenter{\hbox{\includegraphics[width=0.7\textwidth]{images/temp/dmrg-two-site-svd.png}}}$
  \end{tabular}
...
\begin{equation}
  \vcenter{\hbox{\includegraphics[width=0.7\textwidth]{images/temp/itebd-w-matrices.png}}}
\end{equation}
...
\begin{figure}[htb]
  \centering
  \begin{tabular}{cc}
    \includegraphics[height=6.5cm]{images/temp/itebd-one-point-function.png} &
...
\begin{enumerate}
  \item 利用 SVD 对原始的张量单元 $A^{(0)}$ 进行分解：
    \begin{align}
      & \vcenter{\hbox{\includegraphics[width=0.8\linewidth]{images/temp/trg-factorizing-svd.png}}}
        \label{eq:trg-factorizing-svd} \\
      & \vcenter{\hbox{\includegraphics[width=0.65\linewidth]{images/temp/trg-factorizing.png}}}
    \end{align}
    在做 SVD 时需要对奇异值进行截断，即只保留 $\chi$ 个最大奇异值，这样可以保证 TRG 的计算开销始终在可控范围内。

  \item 把得到的三角形张量缩并为新的张量单元 $A^{(1)}$：
    \begin{equation}
      \vcenter{\hbox{\includegraphics[width=0.35\linewidth]{images/temp/trg-group.png}}}
    \end{equation}
  ...
  \item 对最终得到的不动点张量进行求迹操作即可得到配分函数 $Z$：
    \begin{equation}
      Z = \sum_{i,j} A^{(N)}_{ijij}
        = \vcenter{\hbox{\includegraphics[width=2cm]{images/temp/trg-double-trace.png}}}
...
\begin{figure}[htb]
  ...
  \caption{利用 TRG 算法计算二维 Ising 模型的配分函数、能量及热容。可以发现在临界点处 TRG 的计算结果与精确值（用虚线标出）有一定差异。}
  \label{fig:trg-ising}
\end{figure}
...
\begin{equation}
  \delta = \vcenter{\hbox{\includegraphics[width=6cm]{images/temp/tnr-truncation-error.png}}}
\end{equation}
