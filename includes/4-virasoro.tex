...
\begin{figure}[ht]
  ...
  \caption[Ising 模型中的两点关联函数]{Ising 模型中能动量张量 $T$（左图）和 $L_{-1}\phi_\sigma$（右图）的两点关联函数。为了避免计算本征值时有限尺寸效应导致的误差（影响 $x$ 较小的区间），以及 iTEBD 算法中的长程累积误差和 iMPS 本身结构带来的限制（影响 $x$ 较大的区间），我们只取 $x=10$ 到 100 区间的数据来拟合幂律公式 $y=Ax^{-2\Delta}$，此时分别得到 $\Delta=1.988$ 和 1.106。它们与 Ising 能谱的理论结果（$\Delta=2$ 和 $1+1/8$）基本相符。}
