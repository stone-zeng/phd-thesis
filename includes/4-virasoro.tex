...
如图~\ref{fig:virasoro-construction} 所示，考虑一个以 $A_{ijkl}$ 为单元构成的一般性的张量网络，其中连接维数 $\chi_A=d$。将 4 个 $A$ 张量连成一个环（圆柱），并对其进行精确对角化，即可按照 \ref{subsec:lattice-approximation} 小节中介绍的方案来确定本征态 $\ket{\phi_T}$ 以及相应的能动量张量 $T$。随后，我们可以把得到的 $T$ 插回由 $A$ 构成的圆环（即把 $j$ 位置处的 $A$ 张量用 $T$ 取代），再按照式~\eqref{eq:lattice-virasoro-operators} 乘上对应的系数 $\ee^{\pm2\pi\ii j n/N}$，这样就得到了格点上的 Virasoro 算符 $L_n$ 或 $\bar{L}_n$。
...
在实际计算中，仅使用 4 个张量构成的圆环，无论是确定能动量张量（圆环~I），还是构造 Virasoro 算符（圆环~II），往往都不能达到很好的精度。一种手段是增加圆环的大小，但为了保持张量缩并时的兼容性，圆环 I 的大小 $N_{\text{I}}$ 必须为 4 的倍数。此外在变形和缩并时，指标的顺序也需要特别注意。% TODO: order of legs
另一种手段是对张量 $A$ 进行粗粒近似，如图~\ref{fig:virasoro-blocking} 所示。例如可以将 $4$ 个 $A$（$\eqcolon A^{(0)}$）组合成一个更大的 $A^{(1)}$，也可以重复这一过程使得 $A^{(i)}$ 逐渐接近不动点张量。在这一过程中，连接维数 $\chi_{A^{(i)}}=d^{2^i}$ 会迅速增大，因此需要其进行截断。这实际上就是利用 TRG 或者 TNR 等张量网络算法来寻找不动点张量的过程。
...
当模型具有额外对称性时，相应 CFT 中的 Virasoro 对称性可以推广为 Kac--Moody 对称性。我们可以用完全类似的方法来构造出格点上的 Kac--Moody 算符：
\begin{equation}
