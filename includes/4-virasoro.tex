...
\begin{figure}[ht]
  ...
  \caption[Ising 模型中能动量张量 $T$ 和 $L_{-1}\phi_\sigma$ 的两点关联函数]{Ising 模型中能动量张量 $T$（左图）和 $L_{-1}\phi_\sigma$（右图）的两点关联函数。为了避免计算本征值时有限尺寸效应导致的误差，以及 iTEBD 算法中的长程累积误差，我们只取 $x=10$ 到 100 区间的数据来拟合幂律公式 $y=Ax^{-2\Delta}$，此时分别得到 $\Delta=1.988$ 和 1.106。它们与 Ising 能谱的理论结果（$\Delta=2$ 和 $1+1/8$）基本相符。}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.45\textwidth]{images/temp/ising_lm1.pdf}    \quad
  \includegraphics[width=0.45\textwidth]{images/temp/ising_lbarm1.pdf} \\
  \includegraphics[width=0.45\textwidth]{images/temp/ising_l1.pdf}     \quad
  \includegraphics[width=0.45\textwidth]{images/temp/ising_lbar1.pdf}  \\
  \includegraphics[width=0.45\textwidth]{images/temp/ising_lm2.pdf}    \quad
  \includegraphics[width=0.45\textwidth]{images/temp/ising_lbarm2.pdf} \\
  \includegraphics[width=0.45\textwidth]{images/temp/ising_l2.pdf}     \quad
  \includegraphics[width=0.45\textwidth]{images/temp/ising_lbar2.pdf}
  \caption{Virasoro 算符在 Ising 模型的能谱上的作用示意图。}
  \label{fig:virasoro-ising-spectrum}
\end{figure}
...
\begin{sidewaystable}[ht]
  ...
  \begin{threeparttable}
    \catcode`\"=\active
    % \def"#1"{\textcolor{MaterialBlue700}{\bfseries#1}}
    \def"#1"{\textbf{#1}}
    ...
    \end{tablenotes}
    \caption[Fibonacci 模型的能谱数据]{Fibonacci 模型的能谱数据。真空态及其后代（标记为粗体）通过对角化带有幂等元的转移矩阵确定，见~[[TODO:]]。$n=\infty$ 处的数据通过对尺寸由 12 到 27 的圆柱（转移矩阵）本征值外推得到。}
...
\begin{equation}
  \Delta = A + \frac{B}{n}
\end{equation}
的形式趋近于其热力学极限值\cite{schuler2016universal}，然后我们在能谱中选择相应的数据以使拟合结果最优（即挑选出最接近拟合曲线的数据点，见图~\ref{fig:fib-fitting}）。计算出拟合参数 $A$ 后，令 $n\to\infty$ 即可得到 $\Delta^\infty$。这些数据列在表~\ref{tab:fib-spectrum} 的“$\infty$”列中。进一步，我们还根据式~\eqref{eq:scaling-dimension-rescale} 标定了能谱中的本征值，使得 $\Delta_{\1}=0$、$\Delta_T=2$，从而可以更为准确地估计标度维数。这些数据列在了“调整值”一列。
